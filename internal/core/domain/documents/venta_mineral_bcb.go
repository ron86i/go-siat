...
// FacturaVentaMineralBCB representa la estructura completa de una factura de Venta de Minerales para el Banco Central (Sector 52).
type FacturaVentaMineralBCB struct {
	XMLName           xml.Name                 `json:"-"`
	XmlnsXsi          string                   `xml:"xmlns:xsi,attr" json:"-"`
	XsiSchemaLocation string                   `xml:"xsi:noNamespaceSchemaLocation,attr" json:"-"`
	Cabecera          CabeceraVentaMineralBCB  `xml:"cabecera" json:"cabecera"`
	Detalle           []DetalleVentaMineralBCB `xml:"detalle" json:"detalle"`
}

// CabeceraVentaMineralBCB contiene la información general y técnica de la factura para el BCB.
type CabeceraVentaMineralBCB struct {
...
// DetalleVentaMineralBCB representa un ítem dentro de la factura para el BCB.
type DetalleVentaMineralBCB struct {
	...
	MontoDescuento         int     `xml:"montoDescuento" json:"montoDescuento"` // Fijo 0 según XSD
	SubTotal               float64 `xml:"subTotal" json:"subTotal"`
}
