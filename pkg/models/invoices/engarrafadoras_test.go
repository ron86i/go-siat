...
func TestEngarrafadoras_Computarizada(t *testing.T) {
	tc := setupTestContext(t, siat.ModalidadComputarizada)
	cuis := tc.GetCuis(t)
	cufd, cufdControl := tc.GetCufd(t, cuis)

	fechaEmision := time.Now()
	cuf, _ := utils.GenerarCUF(tc.Nit, fechaEmision, 0, tc.Modalidad, 1, 1, 51, 1, 0, cufdControl)
	...
	cabecera := invoices.NewEngarrafadorasCabeceraBuilder().
	...
	detalle := invoices.NewEngarrafadorasDetalleBuilder().
	...
	factura := invoices.NewEngarrafadorasBuilder().
