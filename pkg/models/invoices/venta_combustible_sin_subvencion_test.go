...
func TestVentaCombustibleSinSubvencion_Electronica(t *testing.T) {
	...
	cuf, _ := utils.GenerarCUF(tc.Nit, fechaEmision, 0, tc.Modalidad, 1, 1, 55, 1, 0, cufdControl)

	cabecera := invoices.NewVentaCombustibleSinSubvencionCabeceraBuilder().
		WithNitEmisor(tc.Nit).
		WithRazonSocialEmisor("ESTACION DE SERVICIO").
		WithMunicipio("SANTA CRUZ").
		WithNumeroFactura(1).
		WithCuf(cuf).
		WithCufd(cufd).
		WithCodigoSucursal(0).
		WithDireccion("AV. CRISTO REDENTOR").
		WithFechaEmision(fechaEmision).
		WithNombreRazonSocial(&nombreRazonSocial).
		WithCodigoTipoDocumentoIdentidad(1).
		WithNumeroDocumento("1234567").
		WithCodigoCliente("CLI-055").
		WithPlacaVehiculo("1234ABC").
		WithCodigoMetodoPago(1).
		WithMontoTotal(200.00).
		WithMontoTotalSujetoIva(200.00).
		WithCodigoMoneda(1).
		WithTipoCambio(1.0).
		WithMontoTotalMoneda(200.00).
		WithLeyenda("Leyenda Combustible").
		WithUsuario("operador01").
		Build()

	detalle := invoices.NewVentaCombustibleSinSubvencionDetalleBuilder().
		WithActividadEconomica("473000").
		WithCodigoProductoSin(12345).
		WithCodigoProducto("GAS-001").
		WithDescripcion("GASOLINA ESPECIAL").
		WithCantidad(53.47594).
		WithUnidadMedida(58).
		WithPrecioUnitario(3.74).
		WithSubTotal(200.00).
		Build()

	factura := invoices.NewVentaCombustibleSinSubvencionBuilder().
