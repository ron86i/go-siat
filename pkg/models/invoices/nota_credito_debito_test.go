...
func TestNotaCreditoDebitoIntegration_Computarizada(t *testing.T) {
	...
	cuf, err := utils.GenerarCUF(nit, fecha, 0, siat.ModalidadComputarizada, siat.EmisionOnline, 3, 47, 1, 0, cufdControl)
	if err != nil {
...
func TestNotaCreditoDebitoIntegration_Electronica(t *testing.T) {
	...
	// 3. Generar CUF real para Nota (Sector 47) Tipo Documento (3)
	cuf, err := utils.GenerarCUF(nit, fecha, 0, siat.ModalidadElectronica, siat.EmisionOnline, 3, 47, 1, 0, cufdControl)
	if err != nil {
		t.Fatalf("error al generar CUF: %v", err)
	}
